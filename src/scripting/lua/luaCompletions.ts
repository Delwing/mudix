...
const MUDLET_GLOBALS: Completion[] = [
    ...
    fn('deleteCommandLine', '(name) → bool', 'Destroy an overlay command line created by createCommandLine. Fires sysCommandLineDeleted(name) on success.'),
    ...
    fn('getMapZoom',           '([areaID]) → zoom',                'Current map zoom = number of map units visible across the viewport\'s shorter edge (Mudlet-compatible). areaID accepted for compat but mudix uses a single shared 2D view; returns nil when no map panel is open.'),
    fn('setMapZoom',           '(zoom [, areaID]) → bool',         'Set the map zoom: how many map units fit across the shorter edge (larger = zoomed out, must be >= 3.0), then redraw. False when no map panel is open or the zoom is invalid.'),
    ...
    fn('getExitStubsNames',    '(id) → {name,...} | (false, errMsg)', 'Get exit stub directions as names ("north"/…/"other"), 1-indexed; (false, errMsg) when the room is missing'),
    ...
    fn('addCustomLine',        '(roomID, id_to|{points}, direction, style, {r,g,b}, arrow) → bool',
       "Add a custom exit line from roomID. id_to is a target room id (same area) or a list of {x,y,z} points. style is one of 'solid line', 'dash line', 'dot line', 'dash dot line', 'dash dot dot line'. False on an invalid room/style/area mismatch."),
    ...
    fn('searchRoom',           '(id) → name | (name[, cs[, exact]]) → {id=name}',
       'By room id → the room name (false on miss). By name → {roomId=name} of rooms whose name matches (case-insensitive substring by default; pass caseSensitive / exactMatch booleans to tighten).'),
    ...
    fn('getHiddenRooms',       '(areaID) → {roomID,…} | (false, errMsg)', 'List the hidden rooms in an area, 1-indexed. Returns (false, errMsg) when the area is missing.'),
    ...
    fn('pauseSounds',         '([channel])',                          'Halt all currently playing sound effects, optionally filtered by tag/channel. Web Audio source nodes cannot truly pause, so this is implemented as an immediate fade-out + stop; re-trigger playSoundFile to "resume". Music tracks are untouched.'),
    fn('pauseMusic',          '([channel])',                          'Halt currently playing music tracks, optionally filtered by tag/channel. Like pauseSounds, this is an immediate fade-out + stop (Web Audio cannot truly pause); re-trigger playMusicFile to "resume". Sound effects are untouched.'),
